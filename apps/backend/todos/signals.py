...
from .models import TodoItem, TodoSubItem, TodoList


def _recalc_item_progress(item: TodoItem) -> None:
    total = item.subitems.count()
    done = item.subitems.filter(is_done=True).count()
    pct = 100 if (total == 0 and item.is_done) else int(round((done / total) * 100)) if total else (100 if item.is_done else 0)
